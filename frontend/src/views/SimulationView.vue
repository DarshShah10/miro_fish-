...
const handleNextStep = (params = {}) => {
  ...
  // Pass custom rounds via query params if available
  if (params.maxRounds) {
...
const checkAndStopRunningSimulation = async () => {
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      // Try and gracefully close simulation environment
      try {
        ...
        // If graceful close exception, try force stop
        await forceStopSimulation()
      }
    } else {
      // Environment not running, but process might still be there, check status
    ...
    // Failure checking environment status doesn't block subsequent flow
