    ...
    <header class="app-header">
      ...
      <div class="header-right">
        <div class="workflow-step">
          ...
          <span class="step-name">Simulation Run</span>
        </div>
    ...
    if (simRes.success && simRes.data) {
      ...
      try {
        ...
        if (configRes.success && configRes.data?.time_config?.minutes_per_round) {
          ...
          addLog(`Time config: ${minutesPerRound.value} mins per round`)
        }
      } catch (configErr) {
        addLog(`Failed to fetch time config, using default: ${minutesPerRound.value} mins per round`)
      }
...
const loadGraph = async (graphId) => {
  // During simulation, don't show full-screen loading for auto-refresh to avoid flickering
