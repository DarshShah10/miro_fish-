            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  After uploading documents, the LLM analyzes content to automatically generate an ontology structure (entity types + relationship types) suitable for public opinion simulation.
                </div>
              </div>
...
const currentPhase = ref(-1) // -1: uploading, 0: ontology generation, 1: graph building, 2: completed
...
const toggleFullScreen = () => {
  ...
  // Wait for transition to finish before re-rendering graph
  setTimeout(() => {
    renderGraph()
  }, 350) 
}
...
const loadProject = async () => {
  try {
    ...
    if (response.success) {
      ...
      // Continue polling ongoing build tasks
...
const startBuildGraph = async () => {
  try {
    ...
    buildProgress.value = {
      progress: 0,
      message: 'Initiating graph build...'
    }
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        console.log('✅ Graph construction completed, loading full data...')
        
        stopPolling()
        stopGraphPolling()
        currentPhase.value = 2
        
        // Update progress display as completed
        buildProgress.value = {
          progress: 100,
...
const renderGraph = () => {
  ...
  // Draw edges (including clickable transparent thick lines)
